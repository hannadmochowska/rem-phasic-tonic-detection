...
4. Detect eye movement (EM) events
   • Identify peaks in the EOG signal above a per-recording amplitude threshold (90th percentile of absolute EOG within REM by default)
...
class Cfg:
    ...
    # EM event definition
    # Phasic peak threshold is computed per-recording from the absolute EOG distribution
    # (90th percentile by default; see `phasic_peak_percentile`). The `phasic_peak_threshold_uV`
    # value is used as a fallback if percentile computation fails.
    phasic_peak_percentile: float = 90.0
...
def phasic_threshold_from_percentile(
    ...
    """Compute per-recording phasic peak threshold from the absolute EOG distribution.

    We take the *absolute* EOG amplitude within REM intervals and return the chosen percentile.
    If REM samples cannot be extracted (e.g., invalid intervals), fall back to `fallback_uV`.
    """
...
def analyze_subject(edf_path: Path, cfg: Cfg):
    ...
    # Per-recording phasic threshold from percentile of absolute EOG within REM
    phasic_thr_uV = phasic_threshold_from_percentile(
        x_uV=x_uV,
        sf=sf,
        rem_intervals=rem,
        percentile=cfg.phasic_peak_percentile,
        fallback_uV=cfg.phasic_peak_threshold_uV,
    )
    cfg_subj = replace(cfg, phasic_peak_threshold_uV=phasic_thr_uV)

    # EM events (EOG)
    ev_idx = detect_em_events(x_uV, sf, cfg_subj)
    ev_t = ev_idx / sf

    # PHASIC
    ph = detect_phasic(ev_t, rem, cfg_subj)

    # TONIC
    to_all = detect_tonic_maxabs(x_uV, sf, rem, cfg_subj)
