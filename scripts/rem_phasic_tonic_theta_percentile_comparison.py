"""
Percentile-Based Threshold Comparison for REM Phasic/Tonic Detection

This script evaluates how different amplitude thresholds influence the classification of REM sleep into phasic and tonic states. Instead of using fixed thresholds, the script derives thresholds from percentiles of the EOG signal distribution for each recording.
The goal is to systematically test multiple threshold combinations and determine which values produce the most reasonable separation of phasic and tonic REM sleep across subjects.

Pipeline Overview:

1. Load sleep recordings
   • Read polysomnography (PSG) EDF files
   • Load corresponding hypnogram annotations
   • Extract EOG channels used for eye movement detection

2. Identify REM sleep segments
   • Parse hypnogram annotations
   • Extract all REM sleep periods with timestamps

3. Compute amplitude distribution
   • Calculate the absolute EOG amplitude during REM sleep
   • Determine percentile-based thresholds from this distribution

4. Generate threshold combinations
   • Phasic thresholds (e.g., high percentiles of EOG amplitude)
   • Tonic thresholds (e.g., low percentiles of EOG amplitude)

5. Detect eye movement events
   • Identify peaks in the EOG signal above the phasic threshold
   • Apply constraints on event duration and minimum separation

6. Classify REM windows
   REM sleep is divided into fixed windows and labeled as:

   • Phasic REM
     Windows containing bursts of eye movement events

   • Tonic REM
     Windows with low EOG amplitude below the tonic threshold

   • Transition REM
     Windows that do not meet criteria for phasic or tonic

7. Compute statistics
   For each subject and threshold combination:
   • Total phasic duration
   • Total tonic duration
   • Percent of REM spent in each state

8. Aggregate results
   • Store results for all recordings
   • Compute average thresholds across subjects

9. Save outputs
   The script produces CSV files containing:
   • REM state summaries for each threshold combination
   • Combined results across recordings
   • Average phasic and tonic thresholds across subjects

Purpose:
This script helps determine suitable percentile-based thresholds for
EOG-based REM substate classification before applying the detection
pipeline to the full dataset.
"""
...
class Cfg:
    ...
    # Dynamic (percentile-based) thresholds computed from EOG histogram (within REM)
    # If enabled, fixed thresholds above are ignored for detection.
    use_dynamic_thresholds: bool = True

    # Percentile sweep settings (single-cutoff thresholds)
    # Phasic events: abs(EOG) >= percentile(abs(EOG) within REM)
    # Tonic windows: max abs(EOG) <= percentile(abs(EOG) within REM)
    phasic_percentiles: Tuple[float, ...] = (80.0, 85.0, 90.0, 95.0)
    tonic_percentiles: Tuple[float, ...] = (10.0, 15.0, 20.0)
    ...
    eeg_only_examples_all_png: str = "fig5_EEG_only_examples_all_subjects__PzOzOnly.png"
...
def _rem_abs_values(x_uV: np.ndarray, sf: float, rem: List[Tuple[float, float]]) -> np.ndarray:
    """Return |EOG| samples restricted to REM intervals (fallback: whole recording)."""
    x_abs = np.abs(x_uV)
    rem_abs = []
    for a, b in rem:
        i0 = int(round(a * sf))
        i1 = int(round(b * sf))
        if i1 > i0 and i0 < x_abs.size:
            rem_abs.append(x_abs[i0:min(i1, x_abs.size)])
    if not rem_abs:
        return x_abs
    return np.concatenate(rem_abs)


def compute_eog_percentile_threshold(
    x_uV: np.ndarray,
    sf: float,
    rem: List[Tuple[float, float]],
    pct: float,
) -> float:
    """Compute a single percentile threshold from |EOG| values within REM."""
    vals = _rem_abs_values(x_uV, sf, rem)
    return float(np.percentile(vals, pct))


def compute_eog_window_max_percentile_threshold(
    x_uV: np.ndarray,
    sf: float,
    rem: List[Tuple[float, float]],
    pct: float,
    ...
    x_abs = np.abs(x_uV)
    win_n = int(round(window_s * sf))
    if win_n <= 1:
        vals = _rem_abs_values(x_uV, sf, rem)
        return float(np.percentile(vals, pct))

    win_max_vals: list[float] = []
    for a, b in rem:
        cur = float(a)
        end = float(b)
        while cur + window_s <= end:
            ...
            if i1 > x_abs.size:
                break
            win_max_vals.append(float(x_abs[i0:i1].max()))
            cur += window_s
    ...
    return float(np.percentile(win_max_vals, pct))



def detect_em_events(
    x_uV: np.ndarray,
    sf: float,
    cfg: Cfg,
    phasic_thr_uV: float | None = None,
) -> np.ndarray:
    x = np.abs(x_uV)

    if cfg.use_dynamic_thresholds:
        if phasic_thr_uV is None:
            raise ValueError("Dynamic thresholds enabled but phasic_thr_uV not provided.")
        above = x >= float(phasic_thr_uV)
    else:
...
def detect_tonic_maxabs(
    ...
    if cfg.use_dynamic_thresholds:
        if tonic_thr_uV is None:
            raise ValueError("Dynamic thresholds enabled but tonic_thr_uV not provided.")
        thr = float(tonic_thr_uV)
    else:
        thr = float(cfg.tonic_max_abs_uV)
...
def save_all_eeg_examples_grid(payloads: List[Dict], cfg: Cfg, out_path: Path, dur_s: float = 10.0, include_eog: bool = True):
    """
    Create the same per-subject example grid as Fig 4, but optionally plot EEG only (no EOG overlay).

    - Rows: subjects
    - Cols: Phasic / Tonic / Transition (bouts defined from EOG-based labeling)
    - Traces: EEG always; EOG only if include_eog=True
    """
    states = [
    ...
    for r, p in enumerate(payloads):
        ...
        # EEG (downstream plotting)
        t_eeg, eeg = p["t_eeg"], p["eeg_uV"]

        # Optional EOG overlay (used only for plotting context)
        if include_eog:
    ...
    if include_eog:
        fig.suptitle(
            f"EEG+EOG example windows — EEG={cfg.main_theta_channel} (downstream), EOG used for labeling",
            y=0.995
        )
    else:
        fig.suptitle(
            f"EEG-only example windows — EEG={cfg.main_theta_channel} (downstream), labels from EOG (not shown)",
            y=0.995
        )
...
def analyze_subject(edf_path: Path, cfg: Cfg, phasic_pct: float, tonic_pct: float):
    # EOG (detection)
    ...
    # Dynamic thresholds from EOG histogram (within REM)
    # Interpreted as SINGLE cutoffs:
    #   phasic events: abs(EOG) >= P{phasic_pct} of abs(EOG) during REM
    #   tonic windows: max abs(EOG) <= P{tonic_pct} of abs(EOG) during REM
    if cfg.use_dynamic_thresholds:
        ph_thr = compute_eog_percentile_threshold(x_uV=x_uV, sf=sf, rem=rem, pct=phasic_pct)
        to_thr = compute_eog_window_max_percentile_threshold(x_uV=x_uV, sf=sf, rem=rem, pct=tonic_pct, window_s=cfg.window_s)

        print(
            f"[{edf_path.name}] Dynamic EOG thresholds (|uV| within REM): "
            f"phasic >= {phasic_pct:.0f}th = {ph_thr:.1f} µV; "
            f"tonic <= {tonic_pct:.0f}th = {to_thr:.1f} µV"
        )
    else:
        ph_thr = None
        to_thr = None
    ...
    # EM events (EOG)
    ev_idx = detect_em_events(x_uV, sf, cfg, phasic_thr_uV=ph_thr)
    ev_t = ev_idx / sf
    ...
    # TONIC
    to_all = detect_tonic_maxabs(x_uV, sf, rem, cfg, tonic_thr_uV=to_thr)
    ...
    summary = {
        ...
        "phasic_percentile": float(phasic_pct),
        "tonic_percentile": float(tonic_pct),
        "phasic_threshold_uV": (round3(ph_thr) if ph_thr is not None else np.nan),
        "tonic_threshold_uV": (round3(to_thr) if to_thr is not None else np.nan),
        "total_rem_s": round3(total_rem_true),
        "total_phasic_s": round3(total_ph),
        "total_tonic_s": round3(total_to),
        "total_transition_s": round3(total_tr),
...
def run_variant(
    ...
    """Run one (phasic_percentile, tonic_percentile) combination.

    Writes per-recording bout CSVs into results_dir, but returns:
      1) a REM_summary DataFrame for this variant
      2) a REM-period-counts DataFrame for this variant

    The caller (main) saves the combined sweep CSVs.
    """
    ...
    for edf in edf_paths:
        try:
            df, summary, payload, rem_period_df = analyze_subject(
                edf, cfg, phasic_pct=phasic_pct, tonic_pct=tonic_pct
            )
        except Exception as e:
            print(f"SKIP (error): {edf.name} -> {type(e).__name__}: {e}")
            failed.append({"variant": variant, "file": edf.name, "error_type": type(e).__name__, "error": str(e)})
            continue

        # Per-bout CSV (kept per variant for inspection)
        df = df.copy()
        df["variant"] = variant
        df["phasic_percentile"] = float(phasic_pct)
        df["tonic_percentile"] = float(tonic_pct)
        ...
        summary["phasic_percentile"] = float(phasic_pct)
        summary["tonic_percentile"] = float(tonic_pct)
        summaries.append(summary)

        rem_period_df = rem_period_df.copy()
        rem_period_df.insert(0, "subject", edf.stem)
        rem_period_df["variant"] = variant
        rem_period_df["phasic_percentile"] = float(phasic_pct)
        rem_period_df["tonic_percentile"] = float(tonic_pct)
    ...
    if failed:
        ...
        print(f"Saved failure log: {failed_csv}")
...
def main():
    ...
    root_results = cfg.folder.parent / "results" / "percentile_sweep"
    root_results.mkdir(parents=True, exist_ok=True)

    all_summaries = []
    all_rem_periods = []

    # Sweep requested percentiles
    for ph_pct in cfg.phasic_percentiles:
        for to_pct in cfg.tonic_percentiles:
            variant = f"ph{int(round(ph_pct))}_to{int(round(to_pct))}"
            results_dir = root_results / variant
            print(f"\n=== Running variant: {variant} (phasic>={ph_pct}th, tonic<={to_pct}th) ===\n")

            df_sum, rem_all = run_variant(
                cfg,
                edf_paths,
                results_dir,
                phasic_pct=float(ph_pct),
                tonic_pct=float(to_pct),
                variant=variant,
            )
            if not df_sum.empty:
                all_summaries.append(df_sum)
            if not rem_all.empty:
                all_rem_periods.append(rem_all)

    # Write ONE combined summary CSV across all variants
    combined_sum = pd.concat(all_summaries, ignore_index=True) if all_summaries else pd.DataFrame()
    out_sum = root_results / "REM_summary__percentile_sweep_combined.csv"
    combined_sum.to_csv(out_sum, index=False)
    print("Saved combined REM summary CSV:", out_sum.resolve())

    # Write ONE combined thresholds-by-variant CSV (mean across recordings)
    # NOTE: thresholds are per-recording because they are computed from each recording's REM distribution.
    # This summary makes it easy to report a single representative (mean) threshold per sweep variant.
    if not combined_sum.empty:
        thr_cols = ["phasic_threshold_uV", "tonic_threshold_uV"]
        need_cols = ["variant", "phasic_percentile", "tonic_percentile", *thr_cols]
        missing = [c for c in need_cols if c not in combined_sum.columns]
        if missing:
            raise RuntimeError(f"Combined summary is missing expected columns: {missing}")

        g = combined_sum.groupby(["variant", "phasic_percentile", "tonic_percentile"], dropna=False)
        thresholds_summary = (
            g.agg(
                n_recordings=("file_name", "count"),
                mean_phasic_threshold_uV=("phasic_threshold_uV", "mean"),
                mean_tonic_threshold_uV=("tonic_threshold_uV", "mean"),
                std_phasic_threshold_uV=("phasic_threshold_uV", "std"),
                std_tonic_threshold_uV=("tonic_threshold_uV", "std"),
            )
            .reset_index()
        )

        # SEM (std / sqrt(n))
        thresholds_summary["sem_phasic_threshold_uV"] = (
            thresholds_summary["std_phasic_threshold_uV"] / np.sqrt(thresholds_summary["n_recordings"].astype(float))
        )
        thresholds_summary["sem_tonic_threshold_uV"] = (
            thresholds_summary["std_tonic_threshold_uV"] / np.sqrt(thresholds_summary["n_recordings"].astype(float))
        )

        # Round for readability
        for c in [
            "mean_phasic_threshold_uV",
            "mean_tonic_threshold_uV",
            "std_phasic_threshold_uV",
            "std_tonic_threshold_uV",
            "sem_phasic_threshold_uV",
            "sem_tonic_threshold_uV",
        ]:
            thresholds_summary[c] = thresholds_summary[c].round(3)

        out_thr = root_results / "EOG_thresholds__percentile_sweep_mean.csv"
        thresholds_summary.to_csv(out_thr, index=False)
        print("Saved mean thresholds-by-variant CSV:", out_thr.resolve())

    # Combined REM-period-counts across all variants
    combined_rem = pd.concat(all_rem_periods, ignore_index=True) if all_rem_periods else pd.DataFrame()
    out_rem = root_results / "REM_period_counts_all_subjects__percentile_sweep_combined.csv"
    combined_rem.to_csv(out_rem, index=False)
    print("Saved combined REM period counts CSV:", out_rem.resolve())
